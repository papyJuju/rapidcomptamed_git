...
void actesdispo::enregistreacte(){
	...
	int indexComboTables = tableComboBox->currentIndex();
    if (!insertintable(indexComboTables,nomacte,description,type,valeurtotale,date))
    {
...
bool actesdispo::insertintable(int tablenumber,const QString & act,const QString & description,const QString & type,const QString & valeurtotale,const QString & date)
{
    ...
    qDebug() << __FILE__ << QString::number(__LINE__) << "number = " << QString::number(tablenumber);
    switch(tablenumber){
        case NGAP :
        ...
        case FORFAITS :
            ...
            qDebug() << __FILE__ << QString::number(__LINE__) << valuestoinsert;
    ...
    bool test = false;
    if (!qyselect.exec(select.arg(fieldact,table)))
    {
          qWarning() << __FILE__ << QString::number(__LINE__) << qyselect.lastError().text() ;
          qWarning() << __FILE__ << QString::number(__LINE__) << qyselect.lastQuery() ;           
        }
    while (qyselect.next())
    {
